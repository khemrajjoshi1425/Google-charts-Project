  ...
  fetch('coffee production.csv')
    ...
    .then((data) => {
      ...
      rows.forEach((row) => {
        ...
        if (columns.length >= 7) {
          const province = columns[1].trim();
          const plantation = parseFloat(columns[4]) || 0;
          const production = parseFloat(columns[5]) || 0;
          const yieldValue = parseFloat(columns[6]) || 0;
          ...
          provinceData[province].Plantation += plantation;
          provinceData[province].Production += production;
          provinceData[province].Yield += yieldValue;
        }
      });

      const datasets = {
        Plantation: Object.entries(provinceData).map(([province, values]) => [province, values.Plantation]),
        Production: Object.entries(provinceData).map(([province, values]) => [province, values.Production]),
        Yield: Object.entries(provinceData).map(([province, values]) => [province, values.Yield]),
      };

      drawGeoChart(datasets.Plantation, 'Plantation (Ha)');

      document.getElementById('dataOption').addEventListener('change', (event) => {
        const selectedOption = event.target.value;
        drawGeoChart(datasets[selectedOption], `${selectedOption} Data`);
      });
    })
...
function drawGeoChart(data, title) {
  const chartData = new google.visualization.DataTable();
  chartData.addColumn('string', 'Province'); // Region ID (string)
  chartData.addColumn('number', 'Value');    // Value (number)

  // *KEY CHANGE*: Use correct region IDs!
  const regionMapping = { // Map province names to region codes
    "Koshi": "NP-1",  // Example: Koshi Province
    "Madhesh": "NP-2", // Example: Madhesh Province
    "Bagmati": "NP-3", // Example: Bagmati Province
    "Gandaki": "NP-4", // Example: Gandaki Province
    "Lumbini": "NP-5", // Example: Lumbini Province
    "Karnali": "NP-6", // Example: Karnali Province
    "Sudurpashchim": "NP-7", // Example: Sudurpashchim Province
    // ... add all the other provinces
  };

  const dataWithRegionCodes = data.map(row => {
    const provinceName = row[0];
    const regionCode = regionMapping[provinceName]; // Get the code
    if (regionCode) {
      return [regionCode, row[1]]; // Use the code in the DataTable
    } else {
      console.error(`No region code found for province: ${provinceName}`);
      return [provinceName, row[1]]; // Fallback: Use name (might not work for coloring)
    }
  });

  chartData.addRows(dataWithRegionCodes); // Add the data *after* mapping
  ...
  const chart = new google.visualization.GeoChart(document.getElementById('coffee_map'));
  ...
  google.visualization.events.addListener(chart, 'regionClick', function(eventData) {
    if (eventData.region) {
      ...
      const clickedRegionData = dataWithRegionCodes.find(row => row[0] === clickedRegion); // Use codes here
      if (clickedRegionData) {
        console.log("Data for clicked region:", clickedRegionData);
        const infoDiv = document.getElementById('regionInfo');
        if (infoDiv) {
          infoDiv.innerHTML = `<h3>${clickedRegion}</h3><p>Value: ${clickedRegionData[1]}</p>`;
        }
      } else {
        console.log("No data found for clicked region.");
      }
    }
  });
